...
Por tanto, crie um programa que verifique e print no console, em linhas separadas, as propriedades desse objeto.
...
apiReturn.each_with_index do |elemento, index|
    if index >= offset and index <= limit
      puts elemento[:title], elemento[:ingredients], elemento[:recipe]
    end
end